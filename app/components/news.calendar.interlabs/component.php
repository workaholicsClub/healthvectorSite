...
$today = $GLOBALS['calendar']['>=DATE_ACTIVE_FROM']?strtotime($GLOBALS['calendar']['>=DATE_ACTIVE_FROM']):time();
...
$trueMoth = IntVal(date("n", time()));
$trueYear = IntVal(date("Y", time()));
$trueDay = IntVal(date("j",  time()));
if($arParams["TYPE"])
{	//Do not show future news
	if($currentYear > $trueYear)
		return;
	if(($currentYear == $trueYear) && ($currentMonth > $trueMoth))
		return;
}
...
if($this->StartResultCache(false, array($currentMonth, $currentYear, $todayYear, $todayMonth, $todayDay)))
{
  ...
  $arResult["TitleMonth"] = GetMessage("IBL_NEWS_CAL_M_".date("n", mktime(0, 0, 0, $todayMonth, 1, $todayYear)));
  $arResult['TitleDay'] = $todayDay;
  $arResult['TrueDay'] = $todayDay == $trueDay && $todayMonth == $trueMoth && $todayYear == $trueYear;
	...
	if($arParams["TYPE"])
	{
		$bCheckNext = mktime(0, 0, 0 ,$currentMonth+1, 1, $currentYear) < mktime(0, 0, 0 ,$trueMoth+1, 1, $trueYear);
	}
	...
	if($bCheckNext)
	{
		...
		if(!$bNextM)
		{
			...
			$dbItems = CIBlockElement::GetList(array(), $arFilter, false, array("nTopCount"=>1), Array("ID", "IBLOCK_ID"));
			if($arItem = $dbItems->GetNext() || mktime(0,0,0,date('m'),1,date('Y')) > mktime(0,0,0,$currentMonth,1,$currentYear))
				$bNextM = true;
		}
	}
	if($bPrevM)
